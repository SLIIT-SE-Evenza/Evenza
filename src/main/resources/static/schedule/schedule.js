...
/**
 * Loads the user stored in the current Spring Security session.
 */
async function loadCurrentUser() {
    try {
        state.currentUser = await api("/api/auth/me");
    } catch (error) {
        // An expired or missing session must return to the login page.
...
/**
 * Shows only the controls allowed for the logged-in role.
 * Backend security still provides the real protection.
 */
function configureRoleInterface() {
    const role = state.currentUser.role;

    const tasksNavigation =
        document.querySelector('[data-view="tasks-view"]');

    const milestonesNavigation =
        document.querySelector('[data-view="milestones-view"]');
    ...
    const overdueButton =
        document.querySelector("#overdue-button");

    const demoDataButton =
        document.querySelector("#demo-data-button");

    // Demo-data creation is available only to administrators.
    if (role !== "ADMIN") {
        demoDataButton?.classList.add("hidden");
    }

    if (role === "CUSTOMER") {
        // Customers have calendar-only access.
        tasksNavigation?.classList.add("hidden");
        milestonesNavigation?.classList.add("hidden");
        newTaskButton?.classList.add("hidden");
        newMilestoneButton?.classList.add("hidden");
        overdueButton?.classList.add("hidden");
    }

    if (role === "INVENTORY_STAFF") {
        // Staff can view assigned tasks and update their status.
        milestonesNavigation?.classList.add("hidden");
        newTaskButton?.classList.add("hidden");
        newMilestoneButton?.classList.add("hidden");
        overdueButton?.classList.add("hidden");
...
async function loadLookups() {
    ...
    state.events = await api("/api/schedule/events");

    // Only managers and administrators may retrieve the staff list.
    if (role === "EVENT_MANAGER" || role === "ADMIN") {
        state.staff = await api("/api/schedule/staff");
    } else if (role === "INVENTORY_STAFF") {
        // Staff need only their own name for assigned-task display.
        state.staff = [state.currentUser];
    } else {
        state.staff = [];
    }
    ...
    elements.setupBanner.classList.toggle(
        "hidden",
...
async function selectInitialEvent() {
    ...
    const savedId = Number(
        localStorage.getItem("evenza.activeEvent")
    );

    const selected =
        state.events.find(event => event.id === savedId)
        || state.events[0];
...
/**
 * Loads only the data permitted for the logged-in role.
 */
async function loadEventData() {
    const role = state.currentUser.role;

    if (role === "CUSTOMER") {
        // Customers cannot load task-management or milestone endpoints.
        state.tasks = [];
        state.milestones = [];
    ...
    if (role === "INVENTORY_STAFF") {
        // Staff receive only tasks assigned to their own account.
        const [assignedTasks, milestones] = await Promise.all([
            api(`/api/tasks/staff/${state.currentUser.id}`),
            api(`/api/milestones/event/${state.activeEventId}`)
        ]);

        state.tasks = assignedTasks.filter(
            task => task.eventId === state.activeEventId
        );
        ...
        localStorage.setItem(
            "evenza.activeEvent",
            String(state.activeEventId)
        );

        renderAll();
        activateView("tasks-view");
        return;
    }

    // Event Managers and Administrators can manage the full event schedule.
...
/**
 * Changes the visible Schedule section through JavaScript.
 */
...
function renderTasks() {
    ...
    const canUpdateStatus =
        canManage || role === "INVENTORY_STAFF";
    ...
    elements.tasksBody.innerHTML = filtered.map(task => {
        ...
        const canComplete =
            canUpdateStatus
            && ["IN_PROGRESS", "OVERDUE"].includes(task.status);
        ...
        const actionButtons = `
            ...
            ${canModify
                ? `<button class="button small ghost"
                     data-task-action="assign"
                     data-id="${task.id}">Assign</button>`
                : ""}
    ...
    elements.tasksEmpty.classList.toggle(
        "hidden",
        filtered.length > 0
    );
}
...
function renderMilestones() {
        ...
        state.milestones.map(milestone => {
            ...
            const actions = canManage
                ? `
                    ...
                    ${!achieved && milestone.status !== "ON_TRACK"
                        ? `<button class="button small ghost"
                                   data-milestone-action="track"
                                   data-id="${milestone.id}">
                               On track
                           </button>`
                        : ""}

                    ${!achieved && milestone.status !== "AT_RISK"
                        ? `<button class="button small ghost"
                                   data-milestone-action="risk"
                                   data-id="${milestone.id}">
                               At risk
                           </button>`
                        : ""}
    ...
    elements.milestonesEmpty.classList.toggle(
        "hidden",
...
function openTaskDialog(task = null) {
    ...
    creationFields.classList.remove("hidden");
    creationFields.querySelectorAll("input").forEach(input => input.required = true);

    if (task) {
        document.querySelector("#task-start").value = localDateTimeValue(task.startTime);
        document.querySelector("#task-end").value = localDateTimeValue(task.endTime);
        document.querySelector("#task-deadline").value = localDateTimeValue(task.deadline);
        document.querySelector("#task-milestone").value = task.milestoneId || "";
        document.querySelector("#task-staff").value = task.assignedStaffId || "";
    } else {
        setSuggestedTaskTimes();
    }
...
async function saveMilestone(event) {
    ...
    if (!payload.name) {
        showToast("Milestone name is required.", true);
        return;
    }
    if (!payload.targetDate || payload.targetDate < dateInputValue(new Date())) {
        showToast("Milestone target date cannot be in the past.", true);
        return;
    }
...
async function handleMilestoneAction(event) {
    ...
    const endpoints = {
        track: {url: `/api/milestones/${milestone.id}/on-track`, method: "PATCH"},
...
async function loadCalendar() {
    ...
    if (!start || !end) {
        showToast("Choose both calendar dates.", true);
        return;
    }
    if (end < start) {
        showToast("Calendar end date must be on or after the start date.", true);
        return;
    }
...
function applyInputLimits() {
    const now = new Date();
    const minimumDateTime = dateTimeInputValue(now);
    const today = dateInputValue(now);

    // Prevent selecting past task dates in the calendar controls.
    document.querySelector("#task-start").min = minimumDateTime;
    document.querySelector("#task-end").min = minimumDateTime;
    document.querySelector("#task-deadline").min = minimumDateTime;

    document.querySelector("#milestone-date").min = today;
}

function validateTaskPayload(payload) {
    if (!payload.title) return "Task title is required.";
    if (!payload.startTime || !payload.endTime || !payload.deadline) {
        return "Start time, end time and deadline are required.";
    }

    const start = new Date(payload.startTime);
    const end = new Date(payload.endTime);
    const deadline = new Date(payload.deadline);
    // Reject manually entered past dates.
    if (start < new Date()) {
        return "Task start time cannot be in the past.";
    }

    if (end <= start) return "End time must be after start time.";
    if (deadline < start || deadline > end) {
        return "Deadline must be between the task start and end time.";
    }
    return null;
}
...
function localDateTimeValue(value) { return value ? value.substring(0, 16) : ""; }
